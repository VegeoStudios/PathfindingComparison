class AStarGreedy extends Pathfinder {
  ...
  public AStarGreedy() {
    super("A* - Greedy", "A* algorithm but searches the point closest to the destination. Fastest algorithm but not optimal path.");
  ...
  public void Step() {
    ...
    for (Vector2Int neighbor : neighbors) {
      ...
      if (newPath) {
        fScore[neighbor.x][neighbor.y] = H(neighbor);
